module assert
  ...
  interface assert_x_is_gt_y
     module procedure assert_x_is_gt_y_real, assert_x_is_gt_y_int
  end interface assert_x_is_gt_y
...
contains
  ...
  subroutine assert_x_is_ge_y_real(x,y)
    ...
    if (x < y) then
       err_s = "Logical error - x must be greater than or equal to y."
  ...
  subroutine assert_x_is_gt_y_real(x,y)
    !! Asserts that input argument /(x/) is greater than input argument /(y/)
    real, intent(in) :: x, y
    character(len=256) :: err_s
    
    if (x <= y) then
       err_s = "Logical error - x must be greater than y."
       call add_error_message(err_s)
       call print_error_list_to_shell()
       call terminate_with_failure()
    end if

  end subroutine assert_x_is_gt_y_real

  subroutine assert_x_is_gt_y_int(x,y)
    !! Asserts that input argument /(x/) is greater than input argument /(y/)
    integer, intent(in) :: x, y
    character(len=256) :: err_s
  ...
  end subroutine assert_x_is_gt_y_int
  ...
  subroutine assert_same_bounds(x,y)
    ...
    if (lbound(x,1) /= lbound(y,1) .or. ubound(x,1) /= ubound(y,1)) then
       err_s = "Array error - bounds must begin and end at the same position."
       call add_error_message(err_s)
       call print_error_list_to_shell()
       call terminate_with_failure()
    end if
  ...
  subroutine assert_same_rank(x,y)
    ...
    if (size(x) /= size(y)) then
       err_s = "Array error - ranks must be equivalent."
       call add_error_message(err_s)
       call print_error_list_to_shell()
       call terminate_with_failure()
    end if
